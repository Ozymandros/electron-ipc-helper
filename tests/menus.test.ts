...
describe("validateMenuSpec", () => {
  ...
  it("throws on non-object root", () => {
    expect(() => validateMenuSpec("bad")).toThrow(
      "Menu spec root must be an object",
    );
  });
  ...
  it("throws for invalid item field types", () => {
    expect(() => validateMenuSpec({ items: [{ type: 123 }] })).toThrow(
      ".type must be a string",
    );
    expect(() => validateMenuSpec({ items: [{ label: 123 }] })).toThrow(
      ".label must be a string",
    );
    expect(() => validateMenuSpec({ items: [{ id: 123 }] })).toThrow(
      ".id must be a string",
    );
    expect(() => validateMenuSpec({ items: [{ role: 123 }] })).toThrow(
      ".role must be a string",
    );
    expect(() => validateMenuSpec({ items: [{ accelerator: 123 }] })).toThrow(
      ".accelerator must be a string",
    );
    expect(() => validateMenuSpec({ items: [{ enabled: "yes" }] })).toThrow(
      ".enabled must be a boolean",
    );
    expect(() => validateMenuSpec({ items: [{ visible: "yes" }] })).toThrow(
      ".visible must be a boolean",
    );
    expect(() => validateMenuSpec({ items: [{ checked: "yes" }] })).toThrow(
      ".checked must be a boolean",
    );
    expect(() => validateMenuSpec({ items: [{ actionId: 123 }] })).toThrow(
      ".actionId must be a string",
    );
    expect(() => validateMenuSpec({ items: [{ submenu: "nope" }] })).toThrow(
      ".submenu must be an array",
    );
  });
});
...
describe("loadMenuSpecFromFile", () => {
  it("loads JSON by file extension", async () => {
    const tempDir = await mkdtemp(
      path.join(os.tmpdir(), "message-bridge-menu-"),
    );
  ...
  it("throws for unsupported file extension", async () => {
    ...
    try {
      ...
      await expect(loadMenuSpecFromFile(filePath)).rejects.toThrow(
        "Unsupported menu file extension",
      );
    } finally {
      await rm(tempDir, { recursive: true, force: true });
    }
...
describe("buildMenuTemplate", () => {
  it("creates click handlers from actionId", () => {
    ...
    const openItem = (template[0]?.submenu as unknown[])?.[0] as {
      click?: () => void;
    };
    openItem.click?.();
...
describe("buildMenuTemplate — actions registry", () => {
  ...
  function firstClick(
    items: ReturnType<typeof buildMenuTemplate>,
  ): (() => void) | undefined {
