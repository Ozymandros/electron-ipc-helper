 ...
 * @module transport
 *
 * Transport abstraction for electron-ipc-helper.
 *
...
/**
 * Pluggable transport for electron-ipc-helper.
 *
...
export class IpcTransport implements TransportAdapter {
  ...
  handle(channel: string, handler: TransportHandler): void {
    if (!ipcMain) {
      throw new Error('[electron-ipc-helper] ipcMain is not available in this runtime');
    }
  ...
  invoke(_channel: string, _payload: BridgePayload): Promise<BridgePayload> {
    throw new Error(
      '[electron-ipc-helper] IpcTransport.invoke() is not supported — ' +
